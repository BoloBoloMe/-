...
public class ResponseHelper {
    ...
    public static void sendText(ChannelHandlerContext ctx, HttpResponseStatus status, FullHttpRequest request, String text) {
        FullHttpResponse response = new DefaultFullHttpResponse(
                HttpVersion.HTTP_1_1, status, Unpooled.copiedBuffer(text, CharsetUtil.UTF_8));
        response.headers().set(HttpHeaderNames.CONTENT_TYPE, "text/plain; charset=UTF-8");
        HttpUtil.setContentLength(response, response.content().readableBytes());
        sendAndCleanupConnection(ctx, request, response, true);
    }
     ...
     */
    static void sendAndCleanupConnection(ChannelHandlerContext ctx, FullHttpRequest request, HttpResponse response, boolean forceClose) {
